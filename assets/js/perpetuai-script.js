jQuery(document).ready(function ($) {
	...
	function showCustomAlert(message) {
		$('#renewai-custom-alert-message').text(message);
		$('#renewai-custom-alert').show();
	}

	$('#renewai-custom-alert-close').on('click', function () {
		$('#renewai-custom-alert').hide();
	});
	...
	function displayTitleSuggestions(titles) {
		...
		$('#renewai-title-suggestions').remove();

		var $suggestionsContainer = $('<div id="renewai-title-suggestions"></div>');
		...
		titles.forEach(function (title) {
			...
			var $li = $('<li class="renewai-title-suggestion"></li>');
			var $icon = $(
				'<span class="dashicons dashicons-insert" title="Insert this title"></span>'
			);
			var $titleText = $('<span class="title-text"></span>').text(title);
		...
		$('#renewai-post-creator-meta-box').append($suggestionsContainer);
	}
	...
	function insertTitle(title) {
		...
		if ($titleField.length) {
			...
			$('#renewai-post-creator-meta-box').prepend($notification);
			setTimeout(function () {
